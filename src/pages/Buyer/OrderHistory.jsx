  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] py-12">
      <div className="max-w-6xl mx-auto px-4">
        <h1 className="text-2xl md:text-3xl font-sans-elegant uppercase tracking-wider text-[#2C2420] mb-8 text-center">Mis Pedidos</h1>

        <div className="space-y-6">
          {orders.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
          .map((order) => (
            <div key={order.id} className="bg-white border border-[#E0D6CC]">
              {/* Order Header */}
              <div className="bg-[#F5F0EB] border-b border-[#E0D6CC] p-5">
                <div className="flex flex-wrap justify-between items-center gap-4">
                  <div className="flex gap-8 text-sm font-sans-elegant">
                    <div>
                      <p className="text-[#7A6B5A] text-xs uppercase tracking-wide">Pedido</p>
                      <p className="text-[#2C2420] mt-1">#{order.order_number}</p>
                    </div>
                    <div>
                      <p className="text-[#7A6B5A] text-xs uppercase tracking-wide">Fecha</p>
                      <p className="text-[#2C2420] mt-1">{formatDate(order.created_at, true)}</p>
                    </div>
                    <div>
                      <p className="text-[#7A6B5A] text-xs uppercase tracking-wide">Total</p>
              ...
              <div className="p-5">
                {order.items.map((item, index) => (
                  <div key={index} className="flex justify-between items-center py-4 border-b border-[#E0D6CC] last:border-b-0">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-sans-elegant text-sm text-[#2C2420] uppercase tracking-wide">{item.product_name}</p>
                        <p className="text-xs text-[#7A6B5A] font-sans-elegant mt-1">Cantidad: {item.quantity}</p>
                      </div>
                    </div>
                    <p className="font-sans-elegant text-[#2C2420]">${item.price}</p>
                  </div>
                ))}
              </div>
