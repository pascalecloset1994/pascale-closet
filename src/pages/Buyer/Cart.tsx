      ...
      <div className="min-h-screen bg-background py-16">
        <div className="max-w-4xl mx-auto px-4">
          <div className="bg-card border border-border p-16 text-center">
            <div className="text-5xl mb-6 flex justify-center items-center text-border">
              <svg
    ...
    <div className="min-h-screen bg-background py-12">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="text-center mb-12">
          <h1 className="text-2xl md:text-3xl font-sans-elegant uppercase tracking-wider text-foreground">
            Carrito de Compras
          </h1>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-card border border-border">
              {cartItems.map((item: Product, index: number) => {
                ...
                return (
                  <div
                    key={item.id}
                    className={`p-6 ${index !== cartItems.length - 1 ? "border-b border-border" : ""}`}
                  >
                    <div className="sm:flex gap-6">
                      {/* Product Image */}
                      <div className="w-28 h-28 bg-secondary flex-shrink-0 flex items-center justify-center overflow-hidden">
                        {item.image ? (
                          <img
                            src={images[0]}
                            alt={item.name}
                      ...
                      <div className="flex-1 mt-4 sm:mt-0">
                        <Link
                          to={`/product/${item.id}`}
                          className="text-foreground hover:text-muted-foreground font-sans-elegant text-sm uppercase tracking-wide transition-colors duration-300"
                        >
                          {item.name}
                        </Link>
                        <p className="text-xs text-muted-foreground font-sans-elegant mt-1 tracking-wide uppercase">
                        ...
                        <div className="flex items-center gap-6 mt-4">
                          ...
                          <div className="flex items-center border border-border">
                            ...
                            <span className="px-5 py-2 border-x border-border font-sans-elegant text-foreground">
                              {item.quantity}
                            </span>
                            <button
                              onClick={() => {
                          ...
                          <button
                            ...
                            className="text-muted-foreground hover:text-foreground text-xs font-sans-elegant tracking-wide uppercase transition-colors duration-200"
                          >
                            Eliminar
                          </button>
                        </div>
                      ...
                      <div className="text-right mt-4 sm:mt-0">
                        <p className="text-lg font-sans-elegant text-foreground">
                          $
                          {(
          ...
          <div className="lg:col-span-1">
            <div className="bg-card border border-border p-8 sticky top-4">
              <h2 className="text-sm font-sans-elegant text-foreground uppercase tracking-wider mb-6 pb-4 border-b border-border">
                Resumen del Pedido
              </h2>
              ...
              <div className="space-y-4 mb-8">
                ...
                {getCartTotal() < MAX_PAYMENT && (
                  <div className="bg-secondary border border-border p-2 text-pretty">
                    <small className="text-muted-foreground font-sans-elegant leading-relaxed font-semibold">
              ...
              <button
                ...
                className="relative group w-full py-4 border border-foreground text-foreground font-sans-elegant text-xs tracking-[0.15em] uppercase hover:text-background transition-all duration-300 overflow-hidden"
              >
                <div className="absolute -z-10 top-0 left-0 bg-foreground w-full h-full -translate-y-[100%] group-hover:translate-y-[0] transition-transform duration-800"></div>
                <div className="absolute -z-10 top-0 left-0 bg-foreground w-full h-full translate-y-[100%] group-hover:translate-y-[0] transition-transform duration-800"></div>
                Finalizar Compra
              </button>

              <div className="mt-8 p-5 bg-secondary border border-border">
                <p className="text-xs text-muted-foreground font-sans-elegant leading-relaxed">
                  <span className="flex gap-1.5 items-center text-foreground font-medium mb-1">
